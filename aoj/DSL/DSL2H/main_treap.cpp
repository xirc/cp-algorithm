...
struct Query {
    const Data id = { 0, 0, 0, std::numeric_limits<long long>::max() };
    Data operator()(const Data& node, const Data& lhs, const Data& rhs) const {
        Data ans;
        ans.index = node.index;
...
int main() {
    ...
    Treap<Data,long long> treap(query, update, lazy);
    for (int i = 0; i < N; ++i) {
    ...
    for (int i = 0; i < Q; ++i) {
        ...
        if (c == 0 ) {
            cin >> s >> t >> x;
            treap.update({ s, 0, 0, 0}, { t+1, 0, 0, 0 }, x);
        } else if (c == 1) {
            cin >> s >> t;
            cout << treap.query({ s, 0, 0, 0}, { t+1, 0, 0, 0 }).minimum << endl;
        } else throw;
    }
