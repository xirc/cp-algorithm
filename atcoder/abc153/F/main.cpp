...
ll solve() {
    ...
    ll ans = 0;
    SegmentTree<ll,ll> tree(N + 1, plus<ll>(), ll(0), plus<ll>(), plus<ll>(), ll(0));
    for (int i = 0; i < N; ++i) {
        tree.update(i, i + 1, XH[i][1]);
    }
    for (int i = 0; i < N; ++i) {
        ll h = tree.query(i, i + 1);
        if (h <= 0) continue;
        ll c = (h + A - 1) / A;
        array<ll,2> ub = { XH[i][0] + 2 * D, numeric_limits<ll>::max() };
        int j = distance(XH.begin(), upper_bound(XH.begin(), XH.end(), ub));
        // Add -c * A to values of [i,j)
        tree.update(i, j, - c * A);
        ans += c;
    }
    return ans;
