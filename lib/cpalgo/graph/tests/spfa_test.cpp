...
#include "cpalgo/graph/spfa.hpp"

using namespace std;


TEST(SPFATest, IsEmptyInitially) {
    SPFA solver;
    EXPECT_EQ(0ULL, solver.size());
}

TEST(SPFATest, ShouldComputeAllDistancesOnConnectedGraph) {
    size_t N = 4;
    SPFA solver(N);
...
TEST(SPFATest, ShouldFindNegativeCycle) {
    size_t N = 4;
    SPFA solver(N);
...
TEST(SPFATest, ShouldComputeAllDistancesOnDisconnectedGraph) {
    size_t N = 4;
    SPFA solver(N);
